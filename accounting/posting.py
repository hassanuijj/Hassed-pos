...
class InvoicePostingService:
    """Atomic posting service for sales and purchase invoices."""
    ...
    def post_sale(
        ...
        *,
        ...
        if paid < 0 or paid > total:
            raise ValidationError("قيمة المدفوع غير صحيحة.")
    ...
    def post_purchase(
        ...
        *,
        ...
        cash_account_id: int | None = None,
        tax_input_account_id: int | None = None,
        posted_at: datetime | None = None,
    ):
        at = posted_at or datetime.utcnow()
        invoice = self.session.scalar(
        ...
        for line in lines:
            ...
            unit_cost = net_line / self.dec(line.quantity) if self.dec(line.quantity) else Decimal("0")
        ...
        if tax > 0:
            if tax_input_account_id is None:
