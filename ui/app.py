...
class HassedPOSApp(tk.Tk):
    def __init__(self):
        ...
        self.bootstrap=SystemBootstrap(); self.app=self.bootstrap.initialize(); self.workday=WorkdayService(self.app.db); self._build_login()
    def _clear(self):
    ...
    def _build_dashboard(self):
        self._clear(); self.bind_all("<Control-f>",lambda e:self._quick_search()); self.bind_all("<F5>",lambda e:self._build_dashboard()); self.bind_all("<Control-b>",lambda e:self._backup())
        top=ttk.Frame(self,padding=12); top.pack(fill="x"); ttk.Label(top,text="Hassed POS",font=("Arial",22,"bold")).pack(side="left")
        ttk.Button(top,text="بحث سريع (Ctrl+F)",command=self._quick_search).pack(side="left",padx=10); ttk.Button(top,text="نسخ احتياطي (Ctrl+B)",command=self._backup).pack(side="left"); ttk.Button(top,text="تسجيل الخروج",command=self._build_login).pack(side="right")
        body=ttk.Frame(self,padding=15); body.pack(fill="both",expand=True)
        daily=self.workday.today(); d=self.app.dashboard(); vals=[("مبيعات اليوم",daily["sales"]),("مشتريات اليوم",daily["purchases"]),("مصروفات اليوم",daily["expenses"]),("الصندوق",d["cash"])]
        cards=ttk.Frame(body); cards.pack(fill="x")
        for i,(t,v) in enumerate(vals):
            c=ttk.LabelFrame(cards,text=t,padding=18); c.grid(row=0,column=i,padx=6,sticky="nsew"); ttk.Label(c,text=str(v),font=("Arial",18,"bold")).pack(); cards.columnconfigure(i,weight=1)
        alerts=ttk.LabelFrame(body,text=f"تنبيهات العمل ({len(daily['alerts'])})",padding=10); alerts.pack(fill="x",pady=12)
        if daily["alerts"]:
            for a in daily["alerts"]: ttk.Label(alerts,text=f"• {a['title']}: {a['message']}").pack(anchor="w")
        else: ttk.Label(alerts,text="لا توجد تنبيهات حالية").pack(anchor="w")
        modules=ttk.LabelFrame(body,text="الوحدات",padding=15); modules.pack(fill="both",expand=True,pady=10)
    ...
    def _module(self,name):
        self._clear(); top=ttk.Frame(self,padding=12); top.pack(fill="x"); ttk.Button(top,text="← الرئيسية",command=self._build_dashboard).pack(side="left"); ttk.Label(top,text=name,font=("Arial",20,"bold")).pack(side="right")
        ...
        elif name=="المبيعات": self._transaction(f,"sale")
        elif name=="المشتريات": self._transaction(f,"purchase")
        elif name=="الصندوق": self._cash(f)
        elif name=="المحاسبة": self._accounting(f)
    def _inventory(self,p):
        rows=self.app.master.products(); tree=self._tree(p,["الاسم","الباركود","سعر البيع","الحد الأدنى"])
    ...
    def _transaction(self,p,kind):
        ...
        ttk.Label(box,text="مرجع الفاتورة").grid(row=0,column=0,padx=5,pady=5); ttk.Entry(box).grid(row=0,column=1,padx=5,pady=5)
        ttk.Label(box,text="معرّف العميل/المورد").grid(row=1,column=0,padx=5,pady=5); ttk.Entry(box).grid(row=1,column=1,padx=5,pady=5)
    ...
    def _document_list(self,kind):
        typ="sale" if kind=="sale" else "purchase"; self._show_rows(self.app.db.fetchall("SELECT reference,total,paid,created_at FROM documents WHERE document_type=? ORDER BY created_at DESC",(typ,)))
    def _cash(self,p):
        ttk.Label(p,text=f"رصيد الصندوق: {self.app.finance.cash_balance()}",font=("Arial",20,"bold")).pack(pady=20)
        self._show_rows(self.app.db.fetchall("SELECT reference,transaction_type,amount,description,created_at FROM cash_transactions ORDER BY created_at DESC"))
